...
class RecEval(object):
    ...
    def process_one(self, pred, gt, is_valid):
        assert len(pred.structure) == len(
            gt.structure
        ), "Structures must have the same number of sites."
...
def compute_reconstruction_metrics(
    ...
    all_metrics = {}
    # CSP reconstruction keeps disorder weights fixed, so masks come from raw > 0.
    pred_crys, gt_crys, num_evals = get_Crystal_obj_lists(
        path,
        multi_eval,
        ground_truth_path,
        disorder_mask_source="raw_positive",
    )
    if multi_eval:
