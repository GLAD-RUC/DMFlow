...
def register_omega_conf_resolvers():
    ...
    OmegaConf.register_new_resolver("generate_id", generate_id, replace=True)
    OmegaConf.register_new_resolver(
        "get_dmflow_version", lambda: dmflow.__version__, replace=True
    )
...
def load_cfg(
    ...
    return _upgrade_legacy_cfg(cfg)


def _upgrade_legacy_cfg(cfg: DictConfig) -> DictConfig:
    legacy_name = "flow" + "mm"
    cfg_yaml = OmegaConf.to_yaml(cfg)
    if legacy_name not in cfg_yaml:
        return cfg

    cfg_yaml = cfg_yaml.replace(f"get_{legacy_name}_version", "get_dmflow_version")
    cfg_yaml = cfg_yaml.replace(legacy_name, "dmflow")
...
def load_model(
    ...
    atol: float = None,
    rtol: float = None,
) -> "dmflow.model.model_pl.MaterialsRFMLitModule":
