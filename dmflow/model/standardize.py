...
AFFINE_STATS_DIR = Path(__file__).resolve().parent / "stats"
...
def compute_affine_stats(
    ...
    from dmflow.cfg_utils import init_loaders
    ...
    manifold_getter = ManifoldGetter(
        ...
        weight_manifold=weight_manifold,
    ...
    for _ in tqdm(range(epochs)):
        for batch in tqdm(train_loader):
            ...
            atom_types, frac_coords, lattices, *_ = manifold_getter.flatrep_to_georep(
                x_t,
                dims=dims,
            ...
            (
                ...
                u_lattices,
                *_,
            ) = manifold_getter.flatrep_to_georep(
                u_t,
                dims=dims,
                mask_a_or_f=mask_a_or_f,
            )
...
def get_affine_stats_filename(
    dataset: dataset_options,
    manifold_type: (
        atom_type_manifold_types | coord_manifold_types | lattice_manifold_types
    ),
) -> str:
...
def get_affine_stats(
    ...
    if path is None:
        file = AFFINE_STATS_DIR / get_affine_stats_filename(dataset, manifold_type)
        file = file.resolve()
    else:
        file = Path(path)
        file = file.resolve()
...
if __name__ == "__main__":
    ...
    from dmflow.rfm.manifold_getter import (
        ...
        weight_manifold_types,
    )

    AFFINE_STATS_DIR.mkdir(exist_ok=True)
    file = AFFINE_STATS_DIR / "spd_stats.yaml"
    file = file.resolve()
        ...
        for collect_stats_on in collect_stats_on_options.__args__:
            ...
            if collect_stats_on == "coord":
                ...
                stats = compute_affine_stats(
                    ...
                    weight_manifold="sphere_category",
                )
                ...
                file = AFFINE_STATS_DIR / get_affine_stats_filename(
